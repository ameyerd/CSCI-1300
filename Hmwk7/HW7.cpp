...
int getRating(string username, string title, User users[], Book titles[], int numUsers, int numBooks) {
    ...
    for (int s = 1; s < title.length(); s++) {
        ...
        if (title[s] == 79 && title[s+1] == 102) { // of
            title[s] = tolower(title[s]);
        }
        if (title[s] == 84 && title[4] == 76) { // lord of the rings
            title[s] = tolower(title[s]);
        }
    }
    ...
    while (choice != "5") {
        ...
        int read; 
            case 1:
                ...
                // readBooks(string fileName, Book books[], int numBookStored, int booksArrSize)
                
                readBooks(bookFileName, books , numBooks, bookArrSize);
                
                switch(readBooks(bookFileName, books , numBooks, bookArrSize)) {
            ...
            case 2:
                ...
                //readRatings(string fileName, User users[], int numUsersStored, int usersArrSize, int maxCol)
                
                readRatings(userFileName, users, numUsers, userArrSize, bookArrSize);
                
                switch(readRatings(userFileName, users, numUsers, userArrSize, bookArrSize)) {
            ...
            case 3:
                ...
                //printAllBooks(Book books[], int numBooks)
            ...
            case 4:
                ...
                // getRating(string username, string title, User users[], Book titles[], int numUsers, int numBooks)
                getRating(userName, bookTitle, users, books, numUsers, numBooks);
                
                switch(getRating(userName, bookTitle, users, books, numUsers, numBooks)) {
                    case 0: cout << userName << " has not rated " << bookTitle << endl; 
                        break;
                    case -3: cout << userName << " or " << bookTitle << " does not exist" << endl;
                        break;
                    default: cout << userName << " rated " << bookTitle << " with " << getRating(userName, bookTitle, users, books, numUsers, numBooks)  << endl;
                        break;
                }
